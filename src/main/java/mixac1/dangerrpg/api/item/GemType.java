...
public abstract class GemType
{
    ...
    public boolean hasIt(final ItemStack stack) {
        return ((RPGDataRegister<Item, Data, TransferKey, TransferData>)RPGCapability.rpgItemRegistr).isActivated(stack.getItem()) && ((HashMap<K, RPGItemRegister.RPGItemData>)RPGCapability.rpgItemRegistr).get(stack.getItem()).gems.containsKey(this);
    }
    ...
    public boolean isTrueGem(final Gem gem, final ItemStack stack) {
        return this == gem.getGemType() && (gem.itemTypes.isEmpty() || gem.itemTypes.contains(((HashMap<K, RPGItemRegister.RPGItemData>)RPGCapability.rpgItemRegistr).get(stack.getItem()).itemType));
    }
    ...
    public void attach(final ItemStack dest, final List<ItemStack> src) {
        ...
        for (int max = ((HashMap<K, RPGItemRegister.RPGItemData>)RPGCapability.rpgItemRegistr).get(dest.getItem()).gems.get(this).value1, i = 0; i < src.size() && i < max; ++i) {
    ...
    public List<ItemStack> get(final ItemStack stack) {
        if (this.hasIt(stack)) {
            ...
            while (it.hasNext()) {
                ...
                if (itStack == null || !(itStack.getItem() instanceof Gem) || !this.isTrueGem((Gem)itStack.getItem(), stack)) {
                    it.remove();
                }
            }
            return stacks;
    ...
    public List<ItemStack> getRaw(final ItemStack stack) {
        ...
        for (int max = ((HashMap<K, RPGItemRegister.RPGItemData>)RPGCapability.rpgItemRegistr).get(stack.getItem()).gems.get(this).value1, i = 0; i < nbtList.tagCount() && i < max; ++i) {
