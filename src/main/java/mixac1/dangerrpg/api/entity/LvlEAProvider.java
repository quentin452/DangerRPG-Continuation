...
public class LvlEAProvider<Type>
{
        ...
        if (RPGEntityProperties.isServerSide(target)) {
            ...
            if (isUp) {
                if (lvl < this.maxLvl) {
                    ...
                    EntityAttributes.LVL.addValue((Object)1, target);
                    this.attr.setValue((Object)this.mulValue.up((Type)this.attr.getBaseValue(target), target), target);
                    return true;
                }
            }
            else if (lvl > 0) {
                this.setLvl(lvl - 1, target);
                EntityAttributes.LVL.addValue((Object)(-1), target);
                this.attr.setValue((Object)this.mulValue.down((Type)this.attr.getBaseValue(target), target), target);
                return true;
            }
            return false;
        }
    ...
    public static class DafailtLvlEAProvider extends LvlEAProvider<Float>
    {
        public DafailtLvlEAProvider(final int startExpCost, final int maxLvl, final IMultiplier.Multiplier mulValue) {
            super(startExpCost, maxLvl, (IMultiplier.IMultiplierE<Object>)mulValue, IMultiplier.ADD_1);
        }
    }
}
