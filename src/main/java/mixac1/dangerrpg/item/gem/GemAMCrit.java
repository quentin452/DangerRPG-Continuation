...
public class GemAMCrit extends GemAttackModifier
{
    ...
    public void registerAttributes(final Item item, final RPGItemRegister.RPGItemData map) {
        ...
        map.registerIADynamic(GemAttributes.CHANCE, 0.05f, (IMultiplier.Multiplier)new IMultiplier.MultiplierAdd(0.05f));
        map.registerIADynamic(GemAttributes.PERCENT, 1.0f, (IMultiplier.Multiplier)new IMultiplier.MultiplierAdd(0.0f));
    }
    ...
    public void onEntityHit(final ItemStack gem, final EntityPlayer player, final EntityLivingBase target, final Tuple.Stub<Float> damage, final HashSet<Class<? extends GemAttackModifier>> disableSet) {
        ...
        if (!GemAttackModifier.checkDisabling(disableSet, this.getClass()) && GemAttributes.CHANCE.hasIt(gem) && GemAttributes.PERCENT.hasIt(gem) && GemAttributes.CHANCE.get(gem, player) >= rand) {
