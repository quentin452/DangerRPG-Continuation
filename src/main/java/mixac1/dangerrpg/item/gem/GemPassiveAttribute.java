...
public class GemPassiveAttribute extends Gem
{
    ...
    public GemPassiveAttribute(final String name, final float startValue, final IMultiplier.Multiplier mul, final EAWithIAttr... stats) {
        super(name);
        this.stats = stats;
    ...
    public void activate(final ItemStack gem, final EntityPlayer player) {
        if (GemAttributes.AMOUNT.hasIt(gem) && GemAttributes.UUID.hasIt(gem)) {
            for (final EAWithIAttr stat : this.stats) {
                stat.setModificatorValue(Float.valueOf(GemAttributes.AMOUNT.get(gem, player)), (EntityLivingBase)player, GemAttributes.UUID.getUUID(gem));
            }
        }
    }
