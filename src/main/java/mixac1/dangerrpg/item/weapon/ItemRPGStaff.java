...
public class ItemRPGStaff extends ItemSword implements IRPGItem.IRPGItemStaff, IHasBooksInfo
{
    ...
    public ItemRPGStaff(final RPGToolMaterial toolMaterial, final RPGItemComponent.RPGStaffComponent staffComponent) {
        ...
        this.setUnlocalizedName(RPGItems.getRPGName((RPGItemComponent.RPGToolComponent)this.getItemComponent((Item)this), this.getToolMaterial((Item)this)));
    ...
    public void onPlayerStoppedUsing(final ItemStack stack, final World world, final EntityPlayer player, final int useRemain) {
        if (!world.isRemote && RPGHelper.getUsePower(player, stack, stack.getMaxItemUseDuration() - useRemain, 20.0f, 20.0f) > 0.0f) {
    ...
    public void onUpdate(final ItemStack stack, final World world, final Entity entity, final int par, final boolean isActive) {
        if (world.isRemote && isActive && entity instanceof EntityPlayer) {
            final EntityPlayer player = (EntityPlayer)entity;
            if (player.isUsingItem()) {
                final double power = RPGHelper.getUsePower(player, stack, stack.getMaxItemUseDuration() - player.getItemInUseCount(), 20.0f);
                final int color = RPGHelper.getSpecialColor(stack, 3605646);
                final Vec3 vec = RPGHelper.getFirePoint((EntityLivingBase)player);
                DangerRPG.proxy.spawnEntityFX(RPGEntityFXManager.EntityReddustFXE, vec.xCoord, vec.yCoord + 1.0 - 1.0 * power, vec.zCoord, 0.0, 0.0, 0.0, color);
            }
        }
    }
}
