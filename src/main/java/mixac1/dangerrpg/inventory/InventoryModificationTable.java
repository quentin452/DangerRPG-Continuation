...
public class InventoryModificationTable implements IInventory {
    ...
    private void detachGems(final ItemStack stack) {
        final HashMap<GemType, Tuple.Stub<Integer>> map = (HashMap<GemType, Tuple.Stub<Integer>>) ((RPGItemRegister.RPGItemData) RPGCapability.rpgItemRegistr
            .get((Object) stack.getItem())).gems;
        this.inv = new ItemStack[map.size()][];
        int i = 0;
        for (final Map.Entry<GemType, Tuple.Stub<Integer>> entry : map.entrySet()) {
            final List<ItemStack> list = (List<ItemStack>) entry.getKey()
                .detach(stack);
    ...
    public boolean isItemValidForSlot(final int index, final ItemStack stack) {
        ...
        if (this.main != null && stack.getItem() instanceof Gem) {
            ...
            if (gemType != null) {
                return gemType.isTrueGem((Gem) stack.getItem(), this.main)
                    && ItemAttributes.LEVEL.get(this.main) >= ItemAttributes.LEVEL.get(stack);
            }
        }
        return false;
    }
