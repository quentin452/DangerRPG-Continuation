...
public class RPGEntityProperties implements IExtendedEntityProperties
{
    ...
    public static void register(final EntityLivingBase entity) {
        entity.registerExtendedProperties("RPGEntityProperties", (IExtendedEntityProperties)new RPGEntityProperties(entity));
    }
    ...
    public void rebuildOnDeath() {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            pas.get(rand).up(this.entity, (EntityPlayer)null, false);
            if (pas.get(rand).getLvl(this.entity) <= 1) {
                pas.remove(rand);
            }
        }
    }
