        ...
        public IMessage onMessage(final MsgUseItemExtra message, final MessageContext ctx) {
            ...
            if (player.inventory.currentItem >= 0 && player.inventory.currentItem <= 8) {
                ...
                if (stack != null && stack.getItem() instanceof IUseItemExtra) {
                    final ItemStack resStack = ((IUseItemExtra)stack.getItem()).onItemUseExtra(stack, player.worldObj, player);
                    if ((resStack != stack || resStack.stackSize != stack.stackSize) && ((player.inventory.mainInventory[player.inventory.currentItem] = resStack) == null || resStack.stackSize <= 0)) {
