...
public class MsgSyncEA implements IMessage
{
    ...
    public static class Handler implements IMessageHandler<MsgSyncEA, IMessage>
    {
        public IMessage onMessage(final MsgSyncEA msg, final MessageContext ctx) {
            final EntityLivingBase entity = (EntityLivingBase)DangerRPG.proxy.getEntityByID(ctx, msg.nbt.getInteger("id"));
            if (entity != null) {
                final EntityAttribute attr = RPGEntityProperties.get(entity).getEntityAttribute(msg.nbt.getInteger("hash"));
                if (attr != null) {
