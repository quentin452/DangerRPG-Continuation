...
public class EntityMaterial extends EntityWithStack
{
    ...
    public EntityMaterial(final World world, final EntityLivingBase thrower, final ItemStack stack, final float speed, final float deviation) {
        super(world, thrower, stack, speed, deviation);
    }
    
    public EntityMaterial(final World world, final EntityLivingBase thrower, final EntityLivingBase target, final ItemStack stack, final float speed, final float deviation) {
    ...
    public void applyEntityHitEffects(final EntityLivingBase entity, final float dmgMul) {
        ...
        final DamageSource dmgSource = (this.thrower == null) ? DamageSource.causeThrownDamage((Entity)this, (Entity)this) : ((this.thrower instanceof EntityPlayer) ? DamageSource.causePlayerDamage((EntityPlayer)this.thrower) : DamageSource.causeMobDamage(this.thrower));
        ...
        if (this.thrower != null) {
            ...
            if (knockback != 0) {
                entity.addVelocity((double)(-MathHelper.sin(this.rotationYaw * 3.1415927f / 180.0f) * knockback * 0.5f), 0.1, (double)(MathHelper.cos(this.rotationYaw * 3.1415927f / 180.0f) * knockback * 0.5f));
                this.motionX *= 0.6;
                this.motionZ *= 0.6;
    ...
    protected void onItemPickup(final EntityPlayer player) {
        ((WorldServer)this.worldObj).getEntityTracker().func_151247_a((Entity)this, (Packet)new S0DPacketCollectItem(this.getEntityId(), player.getEntityId()));
    }
