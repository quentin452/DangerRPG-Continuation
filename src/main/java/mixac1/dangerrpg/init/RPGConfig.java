...
public class RPGConfig
{
    ...
    public static class ClientConfig extends RPGConfigCommon
    {
        ...
        public void load() {
            ...
            ClientConfig.d.guiPlayerHUDOffsetX = this.config.getInt("guiPlayerHUDOffsetX", this.category.getName(), ClientConfig.d.guiPlayerHUDOffsetX, 0, Integer.MAX_VALUE, "Change X offset of player's HUD");
            ClientConfig.d.guiPlayerHUDOffsetY = this.config.getInt("guiPlayerHUDOffsetY", this.category.getName(), ClientConfig.d.guiPlayerHUDOffsetY, 0, Integer.MAX_VALUE, "Change Y offset of player's HUD");
            ClientConfig.d.guiPlayerHUDIsInvert = this.config.getBoolean("guiPlayerHUDIsInvert", this.category.getName(), ClientConfig.d.guiPlayerHUDIsInvert, "Change side of player's HUD (true/false)");
            ClientConfig.d.guiEnemyHUDOffsetX = this.config.getInt("guiEnemyHUDOffsetX", this.category.getName(), ClientConfig.d.guiEnemyHUDOffsetX, 0, Integer.MAX_VALUE, "Change X offset of enemy's HUD");
            ClientConfig.d.guiEnemyHUDOffsetY = this.config.getInt("guiEnemyHUDOffsetY", this.category.getName(), ClientConfig.d.guiEnemyHUDOffsetY, 0, Integer.MAX_VALUE, "Change Y offset of enemy's HUD");
            ClientConfig.d.guiEnemyHUDIsInvert = this.config.getBoolean("guiEnemyHUDIsInvert", this.category.getName(), ClientConfig.d.guiEnemyHUDIsInvert, "Change side of enemy's HUD (true/false)");
            ClientConfig.d.guiChargeOffsetX = this.config.getInt("guiChargeOffsetX", this.category.getName(), ClientConfig.d.guiChargeOffsetX, 0, Integer.MAX_VALUE, "Change X offset of charge bar");
            ClientConfig.d.guiChargeOffsetY = this.config.getInt("guiChargeOffsetY", this.category.getName(), ClientConfig.d.guiChargeOffsetY, 0, Integer.MAX_VALUE, "Change Y offset of charge bar");
            ClientConfig.d.guiChargeIsCentered = this.config.getBoolean("guiChargeIsCentered", this.category.getName(), ClientConfig.d.guiChargeIsCentered, "Charge bar need centering (true/false)");
            ClientConfig.d.guiTwiceHealthManaBar = this.config.getBoolean("guiTwiceHealthManaBar", this.category.getName(), ClientConfig.d.guiTwiceHealthManaBar, "Twice health-mana bar (true/false)");
            ClientConfig.d.guiDamageForTestArmor = this.config.getInt("guiDamageForTestArmor", this.category.getName(), ClientConfig.d.guiDamageForTestArmor, 0, Integer.MAX_VALUE, "Default damage value for calculate resistance in armor bar.");
            GuiMode.set(ClientConfig.d.guiDafaultHUDMode = this.config.getInt("guiDafaultHUDMode", this.category.getName(), ClientConfig.d.guiDafaultHUDMode, 0, 3, "Set default HUD mode:\n[0] - normal\n[1] - normal digital\n[2] - simple\n[3] - simple digital\n"));
    ...
    public static class ItemConfig extends RPGConfigCommon
    {
        ...
        public void load() {
            ...
            ItemConfig.d.maxLevel = this.config.getInt("maxLevel", this.category.getName(), ItemConfig.d.maxLevel, 1, Integer.MAX_VALUE, "Set max level of RPG items");
            ItemConfig.d.startMaxExp = this.config.getInt("startMaxExp", this.category.getName(), ItemConfig.d.startMaxExp, 0, Integer.MAX_VALUE, "Set start needed expirience for RPG items");
            ItemConfig.d.expMul = this.config.getFloat("expMul", this.category.getName(), ItemConfig.d.expMul, 0.0f, Float.MAX_VALUE, "Set expirience multiplier for RPG items");
            ItemConfig.d.gemStartLvl = this.config.getInt("gemStartLvl", this.category.getName(), ItemConfig.d.gemStartLvl, 1, Integer.MAX_VALUE, "Set default start gem's level");
        ...
        public void postLoadPost() {
            ...
            this.getPropertyStrings("activeRPGItems", names.toArray(new String[names.size()]), "Set active RPG items (activated if 'isAllItemsRPGable' is false) (true/false)", true);
            ...
            if (!needCustomSetting.isEmpty()) {
                for (final Map.Entry<Item, RPGItemRegister.RPGItemData> item : map.entrySet()) {
                    if (needCustomSetting.contains(item.getKey().delegate.name())) {
                        final ConfigCategory cat = this.config.getCategory(Utils.toString(str, ".", item.getKey().delegate.name()));
                        if (!item.getValue().isSupported) {
                            cat.setComment("Warning: it isn't support from mod");
                        }
                        for (final Map.Entry<ItemAttribute, RPGItemRegister.ItemAttrParams> ia : item.getValue().attributes.entrySet()) {
                        ...
                        for (final Map.Entry<GemType, Tuple.Stub<Integer>> gt : item.getValue().gems.entrySet()) {
                            if (gt.getKey().isConfigurable()) {
                                gt.getValue().value1 = this.config.getInt(gt.getKey().name, cat.getQualifiedName(), (int)gt.getValue().value1, 0, Integer.MAX_VALUE, "");
                            }
                        }
                    }
                }
    ...
    public static class EntityConfig extends RPGConfigCommon
    {
        ...
        public void load() {
            ...
            EntityConfig.d.playerStartManaValue = this.config.getInt("playerStartManaValue", this.category.getName(), EntityConfig.d.playerStartManaValue, 0, Integer.MAX_VALUE, "Set start mana value");
            EntityConfig.d.playerStartManaRegenValue = this.config.getInt("playerStartManaRegenValue", this.category.getName(), EntityConfig.d.playerStartManaRegenValue, 0, Integer.MAX_VALUE, "Set start mana regeneration value");
            EntityConfig.d.playerCanLvlDownAttr = this.config.getBoolean("playerCanLvlDownAttr", this.category.getName(), EntityConfig.d.playerCanLvlDownAttr, "Can player decrease own stats without creative mode? (true/false)");
            EntityConfig.d.playerPercentLoseExpPoints = this.config.getFloat("playerPercentLoseExpPoints", this.category.getName(), EntityConfig.d.playerPercentLoseExpPoints, 0.0f, 1.0f, "Set percent of lose experience points when level down player's stat");
            EntityConfig.d.entityLvlUpFrequency = this.config.getInt("entityLvlUpFrequency", this.category.getName(), EntityConfig.d.entityLvlUpFrequency, 1, Integer.MAX_VALUE, "Set frequency of RPG entity level up");
            EntityConfig.d.entityLvlUpHealthMul = this.config.getFloat("entityLvlUpHealthMul", this.category.getName(), EntityConfig.d.entityLvlUpHealthMul, 1.0f, Float.MAX_VALUE, "Set multiplier of health per level");
        ...
        public void postLoadPost() {
            ...
            this.getPropertyStrings("activeRPGEntities", names.toArray(new String[names.size()]), "Set active RPG entities (activated if 'isAllEntitiesRPGable' is false) (true/false)", true);
        ...
        public void playerConfig() {
            ...
            for (final LvlEAProvider lvlProv : ((RPGEntityRegister.RPGEntityData)RPGCapability.rpgEntityRegistr.get((Object)EntityPlayer.class)).lvlProviders) {
                if (lvlProv.attr.isConfigurable()) {
                    ...
                    lvlProv.startExpCost = this.config.getInt("startExpCost", catStr, lvlProv.startExpCost, 0, Integer.MAX_VALUE, "");
                    if (lvlProv.mulValue instanceof IMultiplier.Multiplier) {
                        lvlProv.mulValue = this.getEAMultiplier(catStr, "value", lvlProv.attr, (IMultiplier.Multiplier)lvlProv.mulValue);
                    }
            ...
            if (!needCustomSetting.isEmpty()) {
                for (final Map.Entry<Class<? extends EntityLivingBase>, RPGEntityRegister.RPGEntityData> entity : map.entrySet()) {
                    ...
                    if (!EntityPlayer.class.isAssignableFrom(entity.getKey()) && needCustomSetting.contains(entityName = EntityList.classToStringMapping.get(entity.getKey()))) {
                        ...
                        for (final Map.Entry<EntityAttribute, RPGEntityRegister.EntityAttrParams> ea : entity.getValue().attributes.entrySet()) {
                            if (ea.getKey().isConfigurable()) {
                                ea.getValue().mulValue = this.getEAMultiplier(cat.getQualifiedName(), ea.getKey().name, ea.getKey(), ea.getValue().mulValue);
                            }
                        }
                    }
                }
