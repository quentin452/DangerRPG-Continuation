...
public abstract class RPGCapability {
    ...
    private static void registerDefaultRPGEntities() {
        ...
        RPGRegister
            .registerRPGEntity((Class) EntitySkeleton.class, (IRPGEntity) new IRPGEntity.RPGRangeEntityMob(2.0f));
        RPGRegister
            .registerRPGEntity((Class) EntityGhast.class, (IRPGEntity) new IRPGEntity.RPGCommonRangeEntity(6.0f));
    ...
    private static void loadEntities() {
        ...
        for (final Map.Entry<Class<? extends EntityLivingBase>, RPGEntityRegister.RPGEntityData> it2 : RPGCapability.rpgEntityRegistr
            .entrySet()) {
            RPGEntityHelper
                .registerEntityDefault((Class) it2.getKey(), (RPGEntityRegister.RPGEntityData) it2.getValue());
            it2.getValue().rpgComponent
                .registerAttributes((Class) it2.getKey(), (RPGEntityRegister.RPGEntityData) it2.getValue());
            if (RPGConfig.EntityConfig.d.isAllEntitiesRPGable || RPGConfig.EntityConfig.activeRPGEntities
                .contains(EntityList.classToStringMapping.get(it2.getKey()))) {
                ((RPGEntityRegister.RPGEntityData) RPGCapability.rpgEntityRegistr
                    .get((Object) it2.getKey())).isActivated = true;
                DangerRPG.infoLog(
                    new Object[] { String.format(
                        "Register RPG entity (sup from mod: %s): %s",
                        it2.getValue().isSupported ? " true" : "false",
                        EntityList.classToStringMapping.get(it2.getKey())) });
            }
        }
        ((RPGEntityRegister.RPGEntityData) RPGCapability.rpgEntityRegistr
            .get((Object) EntityPlayer.class)).isActivated = true;
    }

    private static void loadItems() {
        final Iterator iterator = GameData.getItemRegistry()
            .iterator();
        ...
        for (final Map.Entry<Item, RPGItemRegister.RPGItemData> it : RPGCapability.rpgItemRegistr.entrySet()) {
            ...
            it.getValue().rpgComponent
                .registerAttributes((Item) it.getKey(), (RPGItemRegister.RPGItemData) it.getValue());
            if (it.getKey() instanceof Gem || RPGConfig.ItemConfig.d.isAllItemsRPGable
                ...
                ((RPGItemRegister.RPGItemData) RPGCapability.rpgItemRegistr
                    .get((Object) it.getKey())).isActivated = true;
                DangerRPG.infoLog(
                    new Object[] { String.format(
