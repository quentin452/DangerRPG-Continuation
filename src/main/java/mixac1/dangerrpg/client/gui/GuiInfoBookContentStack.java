...
public class GuiInfoBookContentStack extends GuiInfoBookContent
{
    ...
    public GuiInfoBookContentStack(final Minecraft mc, final int width, final int height, final int top, final int size, final int left, final GuiInfoBook parent, final ItemStack stack) {
    ...
    public void init() {
        ...
        this.addCenteredString(this.stack.getDisplayName().toUpperCase());
        this.addString("");
        ...
        if (this.stack.getItem() instanceof IHasBooksInfo) {
            ...
            if (s != null) {
                this.addCenteredString(DangerRPG.trans("rpgstr.description").toUpperCase());
                this.addString("");
                this.addString(s);
                this.addString("");
            }
        ...
        if (isRPGable) {
            if (!((RPGItemRegister.RPGItemData)RPGCapability.rpgItemRegistr.get((Object)this.stack.getItem())).isSupported) {
                this.addString(DangerRPG.trans("rpgstr.item_not_supported"));
                this.addString("");
            }
            final Set<ItemAttribute> attrs = new LinkedHashSet<ItemAttribute>(RPGItemHelper.getItemAttributes(this.stack));
            ...
            final Set<GemType> set = ((RPGItemRegister.RPGItemData)RPGCapability.rpgItemRegistr.get((Object)item)).gems.keySet();
            ...
            if (this.stack.isItemStackDamageable()) {
                this.addString(String.format("%s: %d/%d", ItemAttributes.DURABILITY.getDispayName(), this.stack.getItemDamage(), this.stack.getMaxDamage()));
            }
            else {
                this.addString(String.format("%s: %s", ItemAttributes.DURABILITY.getDispayName(), DangerRPG.trans("rpgstr.unbreakable")));
            }
    ...
    private void initGem(final ItemStack stack, final GemType gemType) {
        ...
        this.addCenteredString(DangerRPG.trans("rpgstr.gem").toUpperCase());
        this.addString("");
        ...
        this.addString(Utils.toString(ItemAttributes.LEVEL.getDispayName(), ": ", (int)ItemAttributes.LEVEL.get(stack), tmp));
        this.addString("");
        ...
        if (!attrs.isEmpty()) {
            ...
            for (final ItemAttribute iter : attrs) {
                if (iter.isVisibleInInfoBook(stack)) {
                    this.addString(String.format("%s : %s", iter.getDispayName(), iter.getDispayValue(stack, this.player)));
                }
            }
            this.addString("");
        }
    ...
    public class CenteredString
    {
        ...
        public void draw(final int x, final int y, final int color) {
            final String s = GuiInfoBookContentStack.this.mc.fontRenderer.trimStringToWidth(this.str, GuiInfoBookContentStack.this.listWidth);
            GuiInfoBookContentStack.this.mc.fontRenderer.drawString(s, x + (GuiInfoBookContentStack.this.listWidth - GuiInfoBookContentStack.this.mc.fontRenderer.getStringWidth(s)) / 2, y, color);
        }
    }
}
