...
public class GuiInfoBook extends GuiScreen
{
    ...
    public GuiInfoBook(final EntityPlayer player) {
        ...
        this.attributes = (List<LvlEAProvider>)RPGEntityProperties.get(this.target).getLvlProviders();
    }
    ...
    public void setWorldAndResolution(final Minecraft mc, final int width, final int height) {
        ...
        if (this.isTargetPlayer) {
            ...
            this.stacks = new ItemStack[] { tmp.getCurrentEquippedItem(), tmp.getCurrentArmor(3), tmp.getCurrentArmor(2), tmp.getCurrentArmor(1), tmp.getCurrentArmor(0) };
        }
        else {
        ...
        this.content[0] = new GuiInfoBookContentEntity(mc, GuiInfoBook.bookImageW - GuiInfoBook.contentX * 2, 0, this.offsetY + GuiInfoBook.contentY, GuiInfoBook.contentS, this.offsetX + GuiInfoBook.contentX, this);
        for (int i = 1; i < this.content.length; ++i) {
            this.content[i] = new GuiInfoBookContentStack(mc, GuiInfoBook.bookImageW - GuiInfoBook.contentX * 2, 0, this.offsetY + GuiInfoBook.contentY, GuiInfoBook.contentS, this.offsetX + GuiInfoBook.contentX, this, this.stacks[i - 1]);
        }
        this.buttonList.add(this.buttonDown = new GuiInfoBookContentEntity.LevelUpButton(100, false, this.offsetX + GuiInfoBook.contentX + GuiInfoBookContentEntity.butX, this.offsetY + GuiInfoBook.contentY + GuiInfoBook.contentS + GuiInfoBookContentEntity.butY - GuiInfoBookContentEntity.imageH, (GuiInfoBookContentEntity)this.content[0]));
        this.buttonList.add(this.buttonUp = new GuiInfoBookContentEntity.LevelUpButton(101, true, this.offsetX + GuiInfoBook.contentX + GuiInfoBookContentEntity.butX + GuiInfoBookContentEntity.butW, this.offsetY + GuiInfoBook.contentY + GuiInfoBook.contentS + GuiInfoBookContentEntity.butY - GuiInfoBookContentEntity.imageH, (GuiInfoBookContentEntity)this.content[0]));
    ...
    public void initGui() {
        ...
        for (int k = 0; k < 6; ++k) {
            this.buttonList.add(this.butContent[k] = new SelectContentButton(i++, this.offsetX + GuiInfoBook.butContentX + (GuiInfoBook.butContentS + GuiInfoBook.butContentI) * k, this.offsetY + GuiInfoBook.butContentY));
        }
    }
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float par3) {
        ...
        for (int k = 0; k < this.stacks.length; ++k) {
            if (this.stacks[k] != null) {
                ...
                GuiScreen.itemRender.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.getTextureManager(), this.stacks[k], this.butContent[k + 1].xPosition + 2, this.butContent[k + 1].yPosition + 2);
                GuiScreen.itemRender.renderItemOverlayIntoGUI(this.mc.fontRenderer, this.mc.getTextureManager(), this.stacks[k], this.butContent[k + 1].xPosition + 2, this.butContent[k + 1].yPosition + 2);
            }
        }
        GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        this.mc.getTextureManager().bindTexture(GuiInfoBook.TEXTURE);
        this.drawTexturedModalRect(this.offsetX, this.offsetY, 0, 0, GuiInfoBook.bookImageW, GuiInfoBook.bookImageH);
        final String title = Utils.toString(DangerRPG.trans("rpgstr.info_about"), " ", this.target.getCommandSenderName());
        this.fontRendererObj.drawStringWithShadow(title, this.offsetX + (GuiInfoBook.bookImageW - this.fontRendererObj.getStringWidth(title)) / 2, this.offsetY + (GuiInfoBook.titleHeight - this.fontRendererObj.FONT_HEIGHT) / 2 + 2, 16777215);
        ...
        public void drawButton(final Minecraft mc, final int par1, final int par2) {
            if (this.visible) {
                ...
                if (this.id == GuiInfoBook.currContent) {
                    this.drawTexturedModalRect(this.xPosition, this.yPosition, GuiInfoBook.butContentU, GuiInfoBook.butContentV + GuiInfoBook.butContentS, GuiInfoBook.butContentS, GuiInfoBook.butContentS);
                }
                else if (flag && this.enabled) {
                    this.drawTexturedModalRect(this.xPosition, this.yPosition, GuiInfoBook.butContentU, GuiInfoBook.butContentV, GuiInfoBook.butContentS, GuiInfoBook.butContentS);
                }
                if (this.id == 0 || GuiInfoBook.this.stacks[this.id - 1] == null) {
                    this.drawTexturedModalRect(this.xPosition + 2, this.yPosition + 2, GuiInfoBook.emptyIconU + (GuiInfoBook.this.isTargetPlayer ? 0 : 16), GuiInfoBook.emptyIconV + this.id * 16, 16, 16);
                }
            }
        }
