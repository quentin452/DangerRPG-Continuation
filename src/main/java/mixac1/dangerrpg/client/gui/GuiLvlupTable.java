    ...
    public class LevelUpButton extends GuiButton
    {
        ...
        public void drawButton(final Minecraft mc, final int par1, final int par2) {
            ...
            if (this.visible) {
                ...
                mc.getTextureManager().bindTexture(GuiLvlupTable.TEXTURE);
                int color = 9539985;
                this.enabled = GuiLvlupTable.this.inventorySlots.enchantItem((EntityPlayer)mc.thePlayer, 0);
                if (this.enabled) {
                    if (flag) {
                        if (!Mouse.isButtonDown(0)) {
                            this.drawTexturedModalRect(this.xPosition, this.yPosition, GuiLvlupTable.butU, GuiLvlupTable.butV + GuiLvlupTable.butH, this.width, this.height);
                            color = 15724288;
                        }
                    }
                    else {
                        this.drawTexturedModalRect(this.xPosition, this.yPosition, GuiLvlupTable.butU, GuiLvlupTable.butV, this.width, this.height);
                        color = 15724288;
                    }
                }
                ...
                if (!mc.thePlayer.capabilities.isCreativeMode) {
                    ...
                    if (stack != null && ((ContainerLvlupTable)GuiLvlupTable.this.inventorySlots).expToUp >= 0) {
                        ...
                        if (stack.getItem() instanceof Gem) {
                            str = String.format("%d/%d", mc.thePlayer.experienceLevel, ((ContainerLvlupTable)GuiLvlupTable.this.inventorySlots).expToUp);
                        }
                        else {
                            str = String.format("%d/%d", mc.thePlayer.experienceTotal, ((ContainerLvlupTable)GuiLvlupTable.this.inventorySlots).expToUp);
                        }
                        final int strW = mc.fontRenderer.getStringWidth(str);
                        mc.fontRenderer.drawStringWithShadow(str, (strW > GuiLvlupTable.butW) ? (this.xPosition + this.width - strW) : (this.xPosition + (this.width - strW) / 2), this.yPosition + (this.height - mc.fontRenderer.FONT_HEIGHT) / 2 - 15, color);
                    }
                }
            }
        }
