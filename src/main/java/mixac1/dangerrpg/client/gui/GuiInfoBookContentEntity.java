...
public class GuiInfoBookContentEntity extends GuiInfoBookContent {
    ...
    public static final ResourceLocation TEXTURE = new ResourceLocation(
        "DangerRPG:textures/gui/info_book_player_content.png");
    ...
    public GuiInfoBookContentEntity(Minecraft mc, int width, int height, int top, int size, int left,
        GuiInfoBook parent) {
    ...
    public void drawScreen(int mouseX, int mouseY, float par3) {
        ...
        if (currIndex >= 0) {
            ...
            s = mc.fontRenderer.trimStringToWidth(
                lvlProv.attr.getDisplayName()
                    .toUpperCase(),
                titleW);
            mc.fontRenderer.drawStringWithShadow(
                s,
                offsetX + (titleW - mc.fontRenderer.getStringWidth(s) + 4) / 2,
                offsetY + (titleH - mc.fontRenderer.FONT_HEIGHT + 4) / 2,
                0xffffff);
            ...
            if (!lvlProv.isMaxLvl(parent.target)) {
                ...
                mc.fontRenderer.drawStringWithShadow(
                    s,
                    offsetX + (infoW - mc.fontRenderer.getStringWidth(s)) / 2,
                    offsetY + 73,
                    lvlProv.canUp(parent.target, parent.player) ? 0x00bf00 : 0xbf0000);
            } else {
    ...
    public class ContentItem {
        ...
        public void draw(int x, int y, int color1, int color2) {
            ...
            mc.fontRenderer.drawStringWithShadow(
                s,
                x + (listWidth - mc.fontRenderer.getStringWidth(s)) / 2,
                y + (sizeContent - mc.fontRenderer.FONT_HEIGHT) / 2,
                0xffffff);

            s = String.valueOf(attr.getLvl(parent.target));
            int color = attr.isMaxLvl(parent.target) ? 0xefef00
                : attr.canUp(parent.target, parent.player) ? 0x00bf00 : 0xffffff;
    ...
    public static class LevelUpButton extends GuiButton {
        ...
        public void drawButton(Minecraft mc, int par1, int par2) {
            ...
            if (visible && parent.currIndex >= 0) {
                ...
                enabled = isUp ? parent.parent.attributes.get(parent.currIndex)
                    .canUp(parent.parent.target, parent.parent.player)
                    : parent.parent.attributes.get(parent.currIndex)
                        .canDown(parent.parent.target, parent.parent.player);
                int color = 0x919191;
        ...
        public boolean mousePressed(Minecraft mc, int x, int y) {
            return super.mousePressed(mc, x, y) && parent.parent.attributes.get(parent.currIndex)
                .tryUp(parent.parent.target, parent.parent.player, isUp);
        }
    }
}
