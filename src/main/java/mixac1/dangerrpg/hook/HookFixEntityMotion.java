...
public class HookFixEntityMotion {
    ...
    public static void toBytes(final FMLMessage.EntitySpawnMessage message, final ByteBuf buf) {
        final Entity entity = (Entity) ReflectionHelper
            .getPrivateValue((Class) FMLMessage.EntityMessage.class, (Object) message, new String[] { "entity" });
        final String modId = (String) ReflectionHelper
            .getPrivateValue((Class) FMLMessage.EntitySpawnMessage.class, (Object) message, new String[] { "modId" });
        final int modEntityTypeId = (int) ReflectionHelper.getPrivateValue(
            (Class) FMLMessage.EntitySpawnMessage.class,
            (Object) message,
            new String[] { "modEntityTypeId" });
        ...
        try {
            ...
            FMLLog.log(
                Level.FATAL,
                (Throwable) e,
                "Encountered fatal exception trying to send entity spawn data watchers",
                new Object[0]);
    ...
    public static void sendLocationToAllClients(final EntityTrackerEntry tracker, final List list) {
        ...
        if (!tracker.isDataInitialized
            || tracker.myEntity.getDistanceSq(tracker.posX, tracker.posY, tracker.posZ) > 16.0) {
        ...
        if (tracker.field_85178_v != tracker.myEntity.ridingEntity
            ...
            tracker
                .func_151259_a((Packet) new S1BPacketEntityAttach(0, tracker.myEntity, tracker.myEntity.ridingEntity));
        }
        if (tracker.myEntity instanceof EntityItemFrame && tracker.ticks % 10 == 0) {
            ...
            if (stack != null && stack.getItem() instanceof ItemMap) {
                ...
                for (final Object obj : list) {
                    ...
                    final Packet packet = Items.filled_map
                        .func_150911_c(stack, tracker.myEntity.worldObj, (EntityPlayer) player);
                    if (packet != null) {
            ...
            || tracker.myEntity.getDataWatcher()
                .hasChanges()) {
                    if (tracker.myEntity.ridingEntity == null) {
                        ...
                        Object object = null;
                        final boolean flag = Math.abs(j2) >= 4 || Math.abs(k2) >= 4
                            || Math.abs(l2) >= 4
                            || tracker.ticks % 60 == 0;
                        final boolean flag2 = Math.abs(l - tracker.lastYaw) >= 4
                            || Math.abs(i2 - tracker.lastPitch) >= 4;
                        ...
                        if (tracker.sendVelocityUpdates) {
                            ...
                            if (d5 > d4 * d4 || (d5 > 0.0 && tracker.myEntity.motionX == 0.0
                                && tracker.myEntity.motionY == 0.0
                                && tracker.myEntity.motionZ == 0.0)) {
                        ...
                        final boolean flag3 = Math.abs(i - tracker.lastYaw) >= 4
                            || Math.abs(j - tracker.lastPitch) >= 4;
                        if (flag3) {
                            tracker.func_151259_a(
                                (Packet) new S14PacketEntity.S16PacketEntityLook(
                                    tracker.myEntity.getEntityId(),
                                    (byte) i,
                                    (byte) j));
                            tracker.lastYaw = i;
    ...
    public static void tryStartWachingThis(final EntityTrackerEntry tracker, final EntityPlayerMP player) {
        if (player != tracker.myEntity) {
            ...
            if (d0 >= -tracker.blocksDistanceThreshold && d0 <= tracker.blocksDistanceThreshold
                && d2 >= -tracker.blocksDistanceThreshold
                && d2 <= tracker.blocksDistanceThreshold) {
                if (!tracker.trackingPlayers.contains(player)
                    ...
                    if (!tracker.myEntity.getDataWatcher()
                        ...
                        player.playerNetServerHandler.sendPacket(
                            (Packet) new S1CPacketEntityMetadata(
                                tracker.myEntity.getEntityId(),
                                tracker.myEntity.getDataWatcher(),
                                true));
                    }
                    if (tracker.myEntity instanceof EntityLivingBase) {
                        final ServersideAttributeMap map = (ServersideAttributeMap) ((EntityLivingBase) tracker.myEntity)
                            .getAttributeMap();
                        final Collection collection = map.getWatchedAttributes();
                        if (!collection.isEmpty()) {
                            player.playerNetServerHandler.sendPacket(
                                (Packet) new S20PacketEntityProperties(tracker.myEntity.getEntityId(), collection));
                        }
                    }
                    tracker.motionX = tracker.myEntity.motionX;
                    tracker.motionY = tracker.myEntity.motionY;
                    ...
                    if (tracker.myEntity.ridingEntity != null) {
                        player.playerNetServerHandler.sendPacket(
                            (Packet) new S1BPacketEntityAttach(0, tracker.myEntity, tracker.myEntity.ridingEntity));
                    }
                    if (tracker.myEntity instanceof EntityLiving
                        && ((EntityLiving) tracker.myEntity).getLeashedToEntity() != null) {
                        player.playerNetServerHandler.sendPacket(
                            (Packet) new S1BPacketEntityAttach(
                                1,
                                tracker.myEntity,
                    ...
                    if (tracker.myEntity instanceof EntityLivingBase) {
                        for (int i = 0; i < 5; ++i) {
                            ...
                            if (stack != null) {
                                player.playerNetServerHandler.sendPacket(
                                    (Packet) new S04PacketEntityEquipment(tracker.myEntity.getEntityId(), i, stack));
                            }
                        }
                    }
    ...
    private static S12PacketEntityVelocity createVelocityPacket(final int entityId, final double motionX,
    ...
    @Hook(returnCondition = ReturnCondition.ALWAYS)
    public static void writePacketData(final S12PacketEntityVelocity packet, final PacketBuffer buf)
        throws IOException {
