...
public class HookEntities
{
    ...
    public static void attackTargetEntityWithCurrentItem(final EntityPlayer player, final Entity entity) {
        ...
        if (entity.canAttackWithItem() && !entity.hitByEntity((Entity)player)) {
            if (entity instanceof EntityLivingBase && stack != null && RPGItemHelper.isRPGable(stack) && (float)PlayerAttributes.SPEED_COUNTER.getValue((EntityLivingBase)player) > 0.0f) {
                return;
            }
            ...
            if (entity instanceof EntityLivingBase) {
                moreDmg = EnchantmentHelper.getEnchantmentModifierLiving((EntityLivingBase)player, (EntityLivingBase)entity);
                knockback += EnchantmentHelper.getKnockbackModifier((EntityLivingBase)player, (EntityLivingBase)entity);
            }
            if (player.isSprinting()) {
                ++knockback;
            }
            if (dmg > 0.0f || moreDmg > 0.0f) {
                final boolean crit = player.fallDistance > 0.0f && !player.onGround && !player.isOnLadder() && !player.isInWater() && !player.isPotionActive(Potion.blindness) && player.ridingEntity == null && entity instanceof EntityLivingBase;
                if (crit && dmg > 0.0f) {
                    dmg *= 1.5f;
                }
                dmg += moreDmg;
                ...
                if (stack != null && entity instanceof EntityLivingBase) {
                    final ItemStackEvent.HitEntityEvent event = new ItemStackEvent.HitEntityEvent(stack, (EntityLivingBase)entity, (EntityLivingBase)player, dmg, knockback, false);
                ...
                if (entity.attackEntityFrom(DamageSource.causePlayerDamage(player), dmg)) {
                    if (entity instanceof EntityLivingBase) {
                        points -= ((EntityLivingBase)entity).getHealth();
                        MinecraftForge.EVENT_BUS.post((Event)new ItemStackEvent.DealtDamageEvent(player, (EntityLivingBase)entity, stack, points));
                    }
                    if (knockback > 0.0f) {
                        entity.addVelocity((double)(-MathHelper.sin(player.rotationYaw * 3.1415927f / 180.0f) * knockback * 0.5f), 0.1, (double)(MathHelper.cos(player.rotationYaw * 3.1415927f / 180.0f) * knockback * 0.5f));
                        player.motionX *= 0.6;
                        player.motionZ *= 0.6;
