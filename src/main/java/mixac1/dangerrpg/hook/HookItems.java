...
public class HookItems {
    ...
    @Hook(injectOnExit = true, returnCondition = ReturnCondition.ALWAYS)
    public static Multimap getAttributeModifiers(final Item item, final ItemStack stack,
        @Hook.ReturnValue final Multimap returnValue) {
        if (RPGItemHelper.isRPGable(stack) && ItemAttributes.MELEE_DAMAGE.hasIt(stack)) {
            ...
            returnValue.put(
                (Object) SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(),
                (Object) new AttributeModifier(
                    RPGOther.RPGUUIDs.DEFAULT_DAMAGE,
                    "Weapon modifier",
                    (double) ItemAttributes.MELEE_DAMAGE.get(stack),
                    0));
        }
        return returnValue;
    }
    ...
    @Hook(injectOnExit = true, returnCondition = ReturnCondition.ALWAYS)
    public static int getItemEnchantability(final Item item, final ItemStack stack,
        @Hook.ReturnValue final int returnValue) {
    ...
    public static boolean onEntitySwing(final Item item, final EntityLivingBase entity, final ItemStack stack) {
        return entity instanceof EntityPlayer && RPGItemHelper.isRPGable(stack)
            && (float) PlayerAttributes.SPEED_COUNTER.getValue(entity) > 0.0f;
    }

    @Hook(injectOnExit = true, returnCondition = ReturnCondition.ALWAYS)
    public static EnumRarity getRarity(final Item item, final ItemStack stack,
        @Hook.ReturnValue final EnumRarity returnValue) {
        if (RPGItemHelper.isRPGable(stack)
