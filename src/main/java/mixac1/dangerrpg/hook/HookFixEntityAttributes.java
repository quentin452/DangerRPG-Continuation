...
public class HookFixEntityAttributes {
    ...
    public static void onImpact(final EntitySmallFireball that, final MovingObjectPosition mop) {
        if (!that.worldObj.isRemote) {
            if (mop.entityHit != null) {
                ...
                if (!mop.entityHit.isImmuneToFire() && mop.entityHit.attackEntityFrom(
                    DamageSource.causeFireballDamage((EntityFireball) that, (Entity) that.shootingEntity),
                    damage)) {
    ...
    @Hook(returnCondition = ReturnCondition.ALWAYS)
    public static void attackEntityWithRangedAttack(final EntitySkeleton that, final EntityLivingBase entity,
        final float value) {
        final EntityRPGArrow entityarrow = new EntityRPGArrow(
            that.worldObj,
            (ItemStack) null,
            (EntityLivingBase) that,
            entity,
            1.6f,
            (float) (14 - that.worldObj.difficultySetting.getDifficultyId() * 4));
        ...
        if (tmp == -1.0f) {
            entityarrow.phisicDamage = (float) (value * 2.0f + RPGOther.rand.nextGaussian() * 0.25
    ...
    public static boolean attackEntityAsMob(final EntityIronGolem that, final Entity entity) {
        ...
        final int damage = (int) RPGHelper
            .getMeleeDamageHook((EntityLivingBase) that, (float) (7 + that.rand.nextInt(15)));
        final boolean flag = entity
            .attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase) that), (float) damage);
        if (flag) {
