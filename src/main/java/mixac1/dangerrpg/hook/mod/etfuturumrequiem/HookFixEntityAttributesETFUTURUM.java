      ...
      //  else {
            if ((double)that.getHealth() < (double)that.getMaxHealth() * 0.5 && that.rand.nextInt(4) == 0) {
                ...
                if (that.isEntityAlive() && teleported && !prevClosed) {
                    int i = that.worldObj.selectEntitiesWithinAABB(EntityShulker.class, that.boundingBox.expand(8.0, 8.0, 8.0), IEntitySelector.selectAnything).size();
                    float f = (i - 1) / 5.0F;
    ...
    protected boolean tryTeleportToNewPosition(EntityShulker that,Entity entity) {
        if (func_70650_aV() && that.isEntityAlive()) {
            for(int i = 0; i < 5; ++i) {
                ...
                if (newy > 0 && that.worldObj.isAirBlock(newx, newy, newz) && that.worldObj.getCollidingBoundingBoxes(entity, AxisAlignedBB.getBoundingBox((double)newx, (double)newy, (double)newz, (double)(newx + 1), (double)(newy + 1), (double)(newz + 1))).isEmpty()) {
                    boolean flag = false;
                    EnumFacing[] var6 = EnumFacing.values();
                    int var7 = var6.length;

                    for (EnumFacing enumfacing : var6) {
                        if (that.worldObj.isBlockNormalCubeDefault(newx + enumfacing.getFrontOffsetX(), newy + enumfacing.getFrontOffsetY(), newz + enumfacing.getFrontOffsetZ(), false)) {
                            that.getDataWatcher().updateObject(12, (byte) enumfacing.ordinal());
                            flag = true;
                            break;
                        }
                    }
                    ...
                    if (flag) {
                        ...
                        that.getDataWatcher().updateObject(13, (byte)0);
