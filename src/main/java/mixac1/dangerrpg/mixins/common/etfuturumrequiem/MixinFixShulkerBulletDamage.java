...
public abstract class MixinFixShulkerBulletDamage extends Entity {
    ...
    protected void bulletHit(MovingObjectPosition result) {
        if (result.entityHit == null) {
            this.worldObj
                .spawnParticle("largeexplode", this.posX + .5D, this.posY + .5D, this.posZ + .5D, 0.2D, 0.2D, 0.2D);
            ...
            boolean flag = result.entityHit
                .attackEntityFrom(new EntityDamageSourceIndirect("mob", this, this.owner), tmp);
            if (flag) {
                if (result.entityHit instanceof EntityLivingBase) {
                    ((EntityLivingBase) result.entityHit)
                        .addPotionEffect(new PotionEffect(ModPotions.levitation.getId(), 10 * 20));
                }
            }
        }
