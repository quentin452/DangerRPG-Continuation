        ...
        if (hooks != null) {
            try {
                ...
                for (final AsmHook hook : hooks) {
                    DangerRPG.infoLog(new Object[] { String.format("Hook: patching method %s#%s", hook.getTargetClassName(), hook.getTargetMethodName()) });
                }
                ...
                for (final AsmHook hook2 : hooks) {
                    DangerRPG.infoLog(new Object[] { String.format("Warning: unsuccesfull pathing method %s#%s", hook2.getTargetClassName(), hook2.getTargetMethodName()) });
                }
